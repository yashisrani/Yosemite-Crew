...
const CommonTabForPractitioners = ({
    ...
    const [activeKey, setActiveKey] = useState<string | undefined>(defaultActiveKey || tabs[0]?.eventKey);
            ...
            {showStatusSelect && headname !== "Inventory" && (
                <div className="SelectStatus">
                    ...
                    <Dropdown onSelect={handleDropdownSelect}>
                        ...
                        <Dropdown.Menu>
                            {roleOptions.map((opt) => (
                                <Dropdown.Item eventKey={opt.key} key={opt.key} active={role === opt.key}>
                                    {opt.value}
                                </Dropdown.Item>
                            ))}
                        </Dropdown.Menu>
                ...
                {tabs.map((tab) => (
                    ...
                    >
                        {activeKey === tab.eventKey && tab.eventKey && (
                            <PracticeteamtableForBusinessDashboard departmentId={tab.eventKey} role={role} />
                        )}
                    </Tab>
                ))}
            </Tabs>
