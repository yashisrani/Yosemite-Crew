...
const columns = [
  {
    ...
    render: (item: Practitioner) => (
      <Image
        src={
          item.image ||
          "https://d2il6osz49gpup.cloudfront.net/Images/default-avatar.png"
        }
        alt={item.name}
        width={40}
        height={40}
...
const PracticeteamtableForBusinessDashboard = ({
  departmentId,
  role,
}: Readonly<PracticeteamtableForBusinessDashboardProps>) => {
    ...
    const fetchPractitioners = async () => {
      try {
        ...
        const response = await getData(
          `/fhir/v1/getpractionarList?${queryParams.toString()}`
        );
