...
jest.mock("@/app/stores/oldAuthStore", () => ({
  useOldAuthStore: () => mockUseOldAuthStore(),
}));
jest.mock("@yosemite-crew/fhir", () => ({
  convertFHIRToAdminDepartments: (data: any) =>
    data.map((v: any) => ({ _id: v.id, name: v.name })),
...
describe("CompleteProfile Component", () => {
  ...
  it("should pre-fill form from store", async () => {
    mockUseOldAuthStore.mockReturnValue({
      ...
      profile: {
        name: { businessName: "Pre-filled Clinic" },
        progress: 66,
        key: "service",
      },
    });
    render(<CompleteProfile />);
    await waitFor(() =>
      expect(screen.getByDisplayValue("Pre-filled Clinic")).toBeInTheDocument()
    );
  });

  it("should show validation errors on business tab", async () => {
    render(<CompleteProfile />);
    const businessPane = await screen.findByRole("tabpanel", {
      name: /Business Information/i,
    });
    await user.click(
      within(businessPane).getByRole("button", { name: /Get Started/i })
    );
    expect(
      await within(businessPane).findByText(/Country is required/i)
    ).toBeInTheDocument();
  });
  ...
  it("should handle full navigation flow", async () => {
    ...
    await fillBusinessForm();
    await user.click(screen.getByRole("button", { name: /Get Started/i }));
    const addressPane = await screen.findByRole("tabpanel", {
      name: /Address/i,
    });
    await fillAddressForm();
    await user.click(
      within(addressPane).getByRole("button", { name: /Next/i })
    );
    const servicePane = await screen.findByRole("tabpanel", {
      name: /Service and Department/i,
    });
    expect(servicePane).toBeVisible();
    await user.click(
      within(servicePane).getByRole("button", { name: /Back/i })
    );
    expect(
      await screen.findByRole("tabpanel", { name: /Address/i })
    ).toBeVisible();
  });
  ...
  it("should handle dropdowns, searching, and selection", async () => {
    ...
    const addServicesBtn = within(servicePane).getByRole("button", {
      name: "Add Services",
    });
  ...
  it("should handle API failure on final submission", async () => {
    ...
    await user.click(
      within(servicePane).getByRole("button", { name: "Add Services" })
    );
    await user.click(
      screen.getByRole("checkbox", { name: /Internal Medicine/i })
    );
    await user.click(within(servicePane).getByRole("button", { name: "Yes" }));
    await user.click(
      within(servicePane).getByRole("button", { name: "Add Department" })
    );
    await user.click(
      await screen.findByRole("checkbox", { name: /Cardiology/i })
    );
    await user.click(
      within(servicePane).getByRole("button", { name: /Submit/i })
    );
    await waitFor(() => {
