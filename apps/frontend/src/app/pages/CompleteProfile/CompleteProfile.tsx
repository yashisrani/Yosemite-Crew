...
const CompleteProfile = () => {
  ...
  const [servicesList1, setServicesList1] = useState([
    { code: "", display: "" },
  ]);
  ...
  const handleSelectService = useCallback(
    (service: { code: string; display: string }) => {
  ...
  const validateService = () => {
    ...
    if (addDepartment.length === 0)
      newErrors.addDepartment = "At least one department must be selected";
    return newErrors;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        router.push("/empty-dashboard"); // Redirect to dashboard or another page
      }
    } catch (error) {
  ...
  const handlePlaceSelect = () => {
    if (autoCompleteRef.current) {
      ...
      if (!place?.formatted_address) return; // Ensure a valid selection
      ...
      <Container>
        ...
        <div className="AddVetTabDiv">
          <Tab.Container
            activeKey={key}
            onSelect={(k) => {
              if (typeof k === "string") handleTabSelect(k);
            }}
          >
                          ...
                          <Row>
                            <Col md={12}>
                              <LoadScript
                                googleMapsApiKey={
                                  process.env
                                    .NEXT_PUBLIC_BASE_GOOGLE_MAPS_API_KEY as string
                                }
                                libraries={libraries}
                              >
                                <Autocomplete
                                ...
                                >
                                  <label>
                                    <input
                                      type="checkbox"
                                      className="form-check-input"
                                      checked={addDepartment.includes(
                                        service.code
                                      )}
                                      onChange={() =>
                                        handleSelectService1(service)
                                      }
                                    />
                                    <p>{service.display}</p>
                                  </label>
...
type HeadTextProps = {
  readonly Spntext: string;
  readonly blktext: string;
