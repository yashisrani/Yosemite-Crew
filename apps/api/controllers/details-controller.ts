...
import { FhirOrganization,fhirOrganizationSchema,toFhirOrganizationBreeder,toFhirOrganizationGroomer,toFhirOrganizationBoarding} from '@yosemite-crew/fhir';

import { getCognitoUserId } from '../middlewares/authMiddleware';

import VetClinic, { VetClinicDocument } from '../models/vet-clinic';
import breederModel from '../models/breeder-details'; 
...
// Otherwise, if it's static, call directly as below:
 ...
 breeder: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await breederModel.create(breederData);
