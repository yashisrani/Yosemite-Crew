...
const SlotService = {
  getAvailableTimeSlots: async ({ appointmentDate, doctorId }: GetTimeSlotsInput): Promise<{ resourceType: string; type: string; entry: object[] } | { issue: object[] , entry?:object[]} | { status: number; message: string }> => {
