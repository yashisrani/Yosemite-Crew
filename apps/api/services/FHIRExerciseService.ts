...
class formatExerciseToFHIRBundle {
    ...
    static convertExerciseToFHIR(exercises :any[], options : Options =  {}) {
      const {
        page  = 1,
        limit = 10,
      ...
      const entries = exercises.map((exercise :any) => ({
    ...
    static generatePaginationLinks(page :number, limit :number, total : number, type = "", keyword = "") {
        const current = page;
    ...
    static convertExerciseTypeToFHIR(exerciseTypes :any) {
        const entries = exerciseTypes.map((typeDoc :any) => {
