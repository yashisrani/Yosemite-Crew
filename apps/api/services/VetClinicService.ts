...
class VetClinicService {
    async createClinic(fhirData :any, CognitoUser :string) {
      ...
      const clinic = await VetClinic.create({
        ...
        telephone: data.telecom?.find((t :any)=> t.system === 'phone')?.value || '',
        emailAddess: data.telecom?.find((t :any)=> t.system === 'email')?.value || '',
        website: data.telecom?.find((t :any)=> t.system === 'url')?.value || ''
      });
  
      return clinic;
    }

     toFhirOrganization(clinic :any) {
        return {
