...
class DiabetesService {
     ...
     */
    static async createDiabetesRecord(parsedData, images = [], userId = null) {
      const recordPayload = {
        ...parsedData,
        userId,
    ...
    static async getDiabetesLogs(userId,limit = 10, offset = 0) {
        return await diabetesRecords.find({ userId : {$eq: userId }}).skip(offset)
        .limit(limit);
      }
 
    static async deleteDiabetesLogRecord(id) {
