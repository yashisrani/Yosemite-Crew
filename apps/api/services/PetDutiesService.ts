...
class PetDutiesService {
  ...
  static async saveFromFhirObservation(fhirData :any, userId :string) {
    try {
      ...
      const ownerIdExtension = fhirData.extension?.find(
       ( ext  :any)=> ext.url === "http://example.org/fhir/StructureDefinition/ownerId"
      );
      ...
      const repeatTaskComp = fhirData.component?.find((c :any)=>
        c.code?.coding?.some((code :any)=> code.code === "repeat")
      );
      const taskReminderComp = fhirData.component?.find((c :any) =>
        c.code?.coding?.some((code :any) => code.code === "reminder")
      );
      ...
      const syncWithCalendarExt = fhirData.extension?.find(
        (ext :any) => ext.url === "http://example.org/fhir/StructureDefinition/syncWithCalendar"
      );
      const syncWithCalendar = syncWithCalendarExt?.valueBoolean ?? false;
  
      //  Collect other component details
      const taskDetails = (fhirData.component || []).filter((comp :any)=> {
  ...
  static async getDutiesByUserId(userId :string) {
    const records = await sharedRecord.find({ userId: { $eq: userId } });

    return records.map((record :any)=> {
      const components = (record.taskDetails || []).map((detail  :any )=> ({
        code: {
          coding: [
            {
  ...
  static async updateDutyByIdFromFhir(taskId :string, fhirData :any) {
    ...
    const repeatTask = fhirData.component?.find((c  :any) =>
      c.code?.coding?.some((code :any )=> code.code === "repeat")
    )?.valueString || "none";
  
    const taskReminder = fhirData.component?.find((c :any) =>
      c.code?.coding?.some((code :any)=> code.code === "reminder")
    )?.valueString || "none";
    ...
    const taskDetails = (fhirData.component || [])
      ...
      .filter((detail  :any)=> detail.value !== null); // remove null-only entries
    ...
    return {
      ...
      component: [
        ...
        ...(updated.taskDetails || []).map((detail :any) => ({
          code: {
            coding: [
              {
                system: "http://example.org/fhir/task-metadata",
    ...
    static async deletePetDuties(taskId :string) {
    ...
    static async getPetDutiesById(taskId :string) {
        try {
