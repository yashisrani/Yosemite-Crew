...
class VaccinationService {
  static async saveFhirImmunization(data :any, fileName = "",cognitoUserId: string) {
  ...
  static async getVaccinationsByUserId(userId :string, limit = 10, offset = 0, petId :string) {
    ...
    const query : any = { userId }; // Assuming userId is a safe string (e.g., from JWT or trusted source)
  ...
  static async updateVaccinationFromFHIR(id :string, fileName = null, fhirData :any) {
    ...
    const updatedData = {
      ...
      reminder: fhirData.extension?.[0]?.valueBoolean ?? false
    };
  ...
  static async deleteVaccinationRecord(id :string) {
  ...
  static async getRecentVaccinationsReocrds(userId :string, limit = 10, offset = 0) {
  ...
  static convertToFHIR(data :any) {
    ...
    if (Array.isArray(data.vaccineImage) && data.vaccineImage.length > 0) {
      fhirData.extension = data.vaccineImage.map((image :any) => ({
