...
const EditPetDetails = ({navigation, route}) => {
  ...
  const [select, setSelect] = useState(
    petDetails?.genderStatus === 'Neutered' ? 'Yes' : 'No',
  );
  ...
  const handleConfirm = () => {
    navigation.navigate('EditMorePetDetails', {
      choosePetDetail: {
        ...formValue,
        ...choosePetData,
        petImage: image,
    ...
    <KeyboardAwareScrollView bottomOffset={20} style={styles.container}>
      <TouchableOpacity
        ...
        {image ? (
          ...
          <GImage image={petDetails?.petImages} style={styles.profileImage} />
        )}
      </TouchableOpacity>
