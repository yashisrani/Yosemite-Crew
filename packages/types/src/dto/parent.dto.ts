...
const extractAge = (extension?: Extension[]): Parent["age"] | undefined => {
  const ageExtension = extension?.find(
    (item) => item.url === PARENT_AGE_EXTENSION_URL
  );

  const valueInteger =
    ageExtension && typeof ageExtension.valueInteger === "number"
      ? ageExtension.valueInteger
      : undefined;
  ...
  const valueDecimal =
    ageExtension && typeof ageExtension.valueDecimal === "number"
      ? ageExtension.valueDecimal
      : undefined;

  return typeof valueDecimal === "number"
    ? Math.trunc(valueDecimal)
    : undefined;
};
...
export const toParentResponseDTO = (parent: Parent): ParentResponseDTO => {
  ...
  const address = parent.address
    ? toAddressResponseDTO(parent.address)
    : undefined;
